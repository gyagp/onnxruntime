...
TEST(InferenceSessionTests, TestMobilenetV2Version12CudaTest) {
  const auto& sess_env = GetEnvironment();
  SessionOptions so;
  so.session_log_severity_level = 0;
  so.session_logid = "RunXinghuaCudaTest";
  InferenceSession session_object{so, sess_env};
  Status status;
  std::string model_path = "testdata/mobilenetv2-12.onnx";
...
TEST(InferenceSessionTests, TestStableDiffusionCudaTest) {
  const auto& sess_env = GetEnvironment();
  SessionOptions so;
  so.session_log_severity_level = 0;
  so.session_logid = "RunXinghuaCudaTest";
  InferenceSession session_object{so, sess_env};
  Status status;
  std::string model_path = "testdata/stable-diffusion-2-1/unet/model.onnx";

#if USE_TENSORRT
  ASSERT_STATUS_OK(session_object.RegisterExecutionProvider(DefaultTensorrtExecutionProvider()));
#elif USE_CUDA
  ASSERT_STATUS_OK(session_object.RegisterExecutionProvider(DefaultCudaNHWCExecutionProvider()));
#elif USE_ROCM
  ASSERT_STATUS_OK(session_object.RegisterExecutionProvider(DefaultRocmExecutionProvider()));
#endif
...
TEST(InferenceSessionTests, TestModelExternalDataCudaTest) {
  const auto& sess_env = GetEnvironment();
  SessionOptions so;
  so.session_log_severity_level = 0;
  so.session_logid = "RunXinghuaCudaTest";
  InferenceSession session_object{so, sess_env};
  Status status;
  std::string model_path = "testdata/model.onnx";
  //std::string model_path = "testdata/stable-diffusion-2-1/unet/model.onnx";
...
TEST(InferenceSessionTests, TestXinghuaMaxPool) {
  SessionOptions so;

  so.session_log_severity_level = 0;
  so.session_logid = "InferenceSessionTests.TestXinghuaMaxPool";

  InferenceSession session_object{so, GetEnvironment()};
  Status st;
  ASSERT_TRUE((st = session_object.Load("testdata/maxpool.onnx")).IsOK())
      << st.ErrorMessage();
  ASSERT_STATUS_OK(session_object.Initialize());
  ...
  // prepare inputs
  std::vector<int64_t> dims_x = {1, 1, 5};
  std::vector<float> values_x = {1.764052391052246f, 0.40015721321105957f, 0.978738009929657f, 2.2408931255340576f, 1.8675580024719238f};
  OrtValue ml_value;
  CreateMLValue<float>(TestCPUExecutionProvider()->CreatePreferredAllocators()[0], dims_x, values_x,
                       &ml_value);
  NameMLValMap feeds;
  feeds.insert(std::make_pair("input_0", ml_value));
  ...
  // prepare expected inputs and outputs
  std::vector<int64_t> expected_dims_res = {1, 1, 3};
  std::vector<float> expected_values_res = {1.764052391052246f, 2.2408931255340576f, 2.2408931255340576f};
